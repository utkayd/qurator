...
// console calls the same service layer the HTTP API calls — never HTTP itself — through
// adapters in cmd/qurator/console_adapters.go that satisfy these minimal method sets.
// Tests in this package use fakes that implement these interfaces.
...
// implemented (cookie format, JWT claims, etc. belong to the auth package).
type Authenticator interface {
	// SignIn verifies credentials and, on success, sets the session cookie on w.
...
// Code modes mirror domain.ModeDynamic/domain.ModeDirect as plain strings for console
// forms and templates.
const (
	modeDynamic = "dynamic"
	modeDirect  = "direct"
...
// codeMode converts domain.Code.Mode to the string used by console forms and templates.
...
// presentation (a form error, a 409 banner, a redirect to sign-in). The service adapters
// translate service errors to these sentinels.
var (
